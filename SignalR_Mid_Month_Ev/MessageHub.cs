...
namespace SignalR_Mid_Month_Ev
{
    public class MessageHub:Hub
    {
        private static readonly Dictionary<string, string> users = new Dictionary<string, string>();
        private readonly IWebHostEnvironment env;
        public MessageHub(IWebHostEnvironment env) { this.env = env; }
        ...
        public async Task Upload(string user, ImageData data)
        {
            ...
            fs.Close();
            if (data.Filename.Contains(".jpg") || data.Filename.Contains(".png") || data.Filename.Contains(".gif") || data.Filename.Contains(".jpeg"))
                await Clients.All.SendAsync("Message", user, $"<a target='_blank' href='/Images/" +
                    $"{data.Filename}'><img src='/Images/{data.Filename}' width='40px'  class='img-thumbnail circle'/> </a>");
            else await Clients.All.SendAsync("Message", user, $"<a target='_blank' href='/Images/" +
                $"{data.Filename}'><img src='/Images/docs.png' width='40px'  class='img-thumbnail circle'/> </a>");
        }
    }
}
